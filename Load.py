...
option 3 save ss to os and firebase
...
def upload_snapshot_to_firebase(image_path, destination_folder, file_name):
    bucket = storage.bucket()
...
def take_snapshot(face_image, folder_path, label):
    ...
    if label in name:
        if snapshots_taken[label] == 0:
            ...
            # Upload the snapshot to Firebase Storage (registered face to known_faces folder)
            upload_snapshot_to_firebase(snapshot_path, "known_faces", file_name)
        ...
        # Upload the snapshot to Firebase Storage (unregistered face to unknown_faces folder)
        upload_snapshot_to_firebase(snapshot_path, "unknown_faces", file_name)
...
option 4 naka hiawalay na name
...
def take_snapshot(face_image, folder_path, label):
    ...
    if label in name_mapping:
        if snapshots_taken[label] == 0:
            timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
            file_name = f'snapshot_{name_mapping[label]}_{timestamp}.jpg'
            snapshot_path = os.path.join(folder_path, file_name)
            cv2.imwrite(snapshot_path, face_image)
            print('Snapshot saved at:', snapshot_path)
        ...
        not_registered_counter += 1
        timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        file_name = f'snapshot_Not Registered_{not_registered_counter}_{timestamp}.jpg'
        snapshot_path = os.path.join(folder_path, file_name)
        cv2.imwrite(snapshot_path, face_image)
        print('Snapshot saved at:', snapshot_path)
